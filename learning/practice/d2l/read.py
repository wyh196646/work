...
d2l = sys.modules[__name__]#用来找到当前文件下的所有函数，对象，所以正常的写法里可以用d2l.fuc这样的方式进行调用
...
# Defined in file: ./chapter_linear-networks/softmax-regression-scratch.md
class Accumulator:#数据存储格式，相当于将args分开存储，并且加上
...
def evaluate_loss(net, data_iter, loss):
    ...
    metric = d2l.Accumulator(2)  # Sum of losses, no. of example
    #Accumulator是数据记录器，一个类重载了许多功能
    for X, y in data_iter:
        out = net(X)
...
size = lambda x, *args, **kwargs: x.numel(*args, **kwargs)#numel元素个数
...
#一开始没看懂这个reshape的写法，实际上这里reshape这么写的原因是，将
#reshape这样封装以后，传入一个变量，传入要转成的维度，这样就可以将变量进行转化。
